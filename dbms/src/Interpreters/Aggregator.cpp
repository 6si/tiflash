...
void Aggregator::AggProcessInfo::prepareForAgg()
{
    ...
    std::call_once(dks.init_flag, [&]() {
        if (aggregator->params.keys_size == 1 && aggregator->params.src_header.columns() > 0)
        {
            ...
            LOG_DEBUG(
                aggregator->log,
                "Dict-key init: keys_size={} key_type={} inner_type={} is_nullable={} "
                "has_collator={} collation_safe={} isString={} block_rows={}",
            ...
            if (inner_type->isString() && collation_safe)
            {
                ...
                if (dict_col)
                {
                    ...
                    if (dict_size <= Aggregator::DictKeyState::ACTIVATION_THRESHOLD)
                    {
                        ...
                        LOG_INFO(
                            aggregator->log,
                            "Dict-key activated (ColumnDictionary{}): {} groups -> key16 FixedHashMap",
                    ...
                    else
                    {
                        LOG_DEBUG(
                            aggregator->log,
                            "Dict-key skipped: ColumnDictionary dict_size={} exceeds threshold={}",
                            dict_size,
            ...
            else
            {
                LOG_DEBUG(
                    aggregator->log,
                    "Dict-key skipped: isString={} collation_safe={}",
                    inner_type->isString(),
                    collation_safe);
            }
        }
        else
        {
            LOG_DEBUG(
                aggregator->log,
                "Dict-key skipped: keys_size={} header_columns={}",
                aggregator->params.keys_size,
